...
EOO;
     ...
     $form .= <<< EOM
                            ...
                            <button type="submit" name="addWork" class="btn btn-primary">Сохранить</button>
                        </form>
                    </div>
                </div>
 EOM;
...
function updateWorkStatus(int $id, string $work){
    $dbh = connectDB();
    $query = "UPDATE worklist SET  work_name = :work  WHERE work_status  = :work_status;";
    $params = [
        ':work' => $work,
        ':id' => $id,
    ];
    ...
    foreach ($worklist as $row) {
        $html .= <<<EOT
            <li class="list-group-item ">
                {$row['work_name']} 
                <a href="updateWorkStatus.php?id={$row['id']}"class="btn btn-outline-success btn-sm ml-5">
